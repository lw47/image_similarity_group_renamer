...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        self.tree.tag_configure('separator', background='#e8e8e8', foreground='#555555')
        self.tree.tag_configure('image', background='white')
    ...
    def start_scan(self):
        ...
        log(f'扫描参数: folder={folder!r}, threshold={threshold:.2f}, recursive={self.recursive_var.get()}, prefix={self.prefix_var.get().strip() or "Group"}')
        threading.Thread(target=self._scan_worker, args=(threshold,), daemon=True).start()

    def _scan_worker(self, threshold):
        log('后台分析线程启动')
        try:
    ...
    def render_plan(self):
        ...
        for old, new, g, idx, count in self.plan:
            if current_group is not None and g != current_group:
                # 使用真正有文字、有背景的分隔行，而不是空 Treeview 行。
                sep = self.tree.insert(
                    '', 'end',
                    values=('', f'══════ Group-{g:03d} 分组 ══════', '', ''),
                    tags=('separator',)
                )

            iid = self.tree.insert(
                '', 'end',
                values=(f'Group-{g:03d}', old.name, new, count),
                tags=('image',)
            )
