...
def ensure_requirements_file():
    try:
        if not REQUIREMENTS_FILE.exists():
            ...
            log(f'首次运行：已生成依赖文件: {REQUIREMENTS_FILE}')
    except Exception:
...
class App(tk.Tk):
    ...
    def _ascii_art_python_render(self, path, mode, width=120):
        """Use ascii-art-python rendering backend. The package exposes three modes.
        mode: new_skool | old_skool | full_mode
        """
        try:
            import ascii_art_python as aap
        except ImportError as exc:
            raise RuntimeError(
                '未安装 ascii-art-python。请在脚本目录运行：python -m pip install -r requirements.txt'
            ) from exc

        backend = getattr(aap, mode, None)
        if backend is None or not hasattr(backend, 'Image'):
            raise RuntimeError(f'ascii-art-python 当前版本不支持模式: {mode}')

        image_cls = backend.Image
        img = image_cls.from_path(str(path), width=width)
        return str(img)

    def _print_ascii_header(self, title, width=120):
        """Print a simple terminal header for ASCII output."""
        ...
        line = '=' * width
        print('\n' + line, flush=True)
        print(text.center(width), flush=True)
        print(line, flush=True)

    def _print_ascii_python(self, path, title, mode):
        log(f'ASCII 后端: ascii-art-python, mode={mode}, file={path}')
        art = self._ascii_art_python_render(path, mode, width=120)
        self._print_ascii_header(f' {title}: {path.name} ', 120)
        print(art, flush=True)
        print('=' * 120 + '\n', flush=True)

    def context_ascii_art(self):
        """传统 ASCII：ascii-art-python New Skool（密度/明暗模式）。"""
        path = self._context_selected_path()
        if not path:
            return
        log(f'右键菜单：生成传统 ASCII 画: {path}')
        try:
            self._print_ascii_python(path, 'ASCII ART / NEW SKOOL', 'new_skool')
    ...
    def context_unicode_art(self):
        """高清字符画：ascii-art-python Old Skool（边缘检测模式）。"""
        path = self._context_selected_path()
        if not path:
            return
        log(f'右键菜单：生成高清字符画: {path}')
        try:
            self._print_ascii_python(path, 'ASCII ART / OLD SKOOL', 'old_skool')
            self.status.config(text='高清字符画已输出到当前 CMD / PowerShell。')
        except Exception:
            log('高清字符画生成失败')
            traceback.print_exc()

    def context_color_art(self):
        """彩色/混合字符画：ascii-art-python Full Mode（轮廓 + 阴影混合）。"""
        path = self._context_selected_path()
        if not path:
            return
        log(f'右键菜单：生成彩色高清字符画: {path}')
        try:
            self._print_ascii_python(path, 'ASCII ART / FULL MODE', 'full_mode')
